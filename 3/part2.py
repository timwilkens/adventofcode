def solve():
  ...
  # Walk through the squares for each rectangle. If a given rectangle's squares
  # all have a count of 1 it must be non-overlapping with any others.
  for rd in rectangle_details:
    squares_covered = to_covered_squares(rd)
    is_non_overlapping = True
    for square in squares_covered:
      str_desc = square_to_string(square)
      if covered[str_desc] != 1:
...
def validate_rectangle_details(rectangle_details):
  assert rectangle_details[0]['id'] == '1'
  ...
  assert rectangle_details[1280]['id'] == '1281'
